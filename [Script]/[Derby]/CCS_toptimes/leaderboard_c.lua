...
function Leaderboard.render()
	...
	for i = 1, ROWS_TO_SHOW, 1 do
		...
		for j, column in pairs(tableColumns) do
			local value = column.computed(leaderboard1[i], i)
			...
			if column.isValueImage then
				...
				dxDrawImageSection(currentX, currentY + (ROW_SIZE / 2) - (imageSize / 2), imageSize, imageSize, (48) * flagRow, (48) * flagColumn, 48, 48, "images/flags.png")
			else
...
function Leaderboard.update(list, personalBest)
	...
	if not list or not map or not Leaderboard.isRaceMap(map.type) then return end
...
function Leaderboard.toggle()
	...
	if not map or not Leaderboard.isRaceMap(map.type) or isAnimating then return end
